...
class BipedalWalkerContinuous(gym.Env, EzPickle):
    ...
    def my_init(self, params):
        ...
        self.roughness = params['roughness'] if params['roughness'] else 0
        self.obstacle_spacing = max(0.01, params['obstacle_spacing']) if params['obstacle_spacing'] is not None else 8.0
        self.stump_height = [params['stump_height'], 0.1] if params['stump_height'] is not None else None
        self.stump_width = params['stump_width']
        self.stump_rot = params['stump_rot']
        self.hexa_shape = params['poly_shape']
        self.stump_seq = params['stump_seq']
        if params['poly_shape'] is not None:
            self.hexa_shape = np.interp(params['poly_shape'], [0, 4], [0, 4]).tolist()
            assert (len(params['poly_shape']) == 12)
    ...
    def _generate_terrain(self):
        ...
        while x < max_x:
            ...
            if state == GRASS and not oneshot:
                ...
                stump_height = self.stump_height[0][0]
                ...
                t.color1, t.color2 = (1.0, np.clip(delta_pos[0][1] / 3, 0, 1), np.clip(delta_pos[-1][1] / 3, 0, 1)), (
                    0.6, 0.6, 0.6)
                self.terrain.append(t)
    ...
    def _generate_clouds(self):
        ...
        for i in range(self.TERRAIN_LENGTH // 20):
            ...
            poly = [
                (x + 15 * self.TERRAIN_STEP * math.sin(3.14 * 2 * a / 5) + self.np_random.uniform(0,
                                                                                                  5 * self.TERRAIN_STEP),
                 y + 5 * self.TERRAIN_STEP * math.cos(3.14 * 2 * a / 5) + self.np_random.uniform(0,
                                                                                                 5 * self.TERRAIN_STEP))
                for a in range(5)]
    ...
    def draw_walker(self):
        ...
        for x_anchor in self.leg_pairs_x:
            absolute_x = init_x + np.interp(x_anchor, [-1, 1],
                                            [-self.HULL_WIDTH / 2 / self.SCALE, self.HULL_WIDTH / 2 / self.SCALE])
            for i in [-1, +1]:
    ...
    def reset(self):
        ...
        # observation = self._add_C_to_state()
